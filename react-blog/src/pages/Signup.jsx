...
const Signup = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            setLocalError(err?.message || "Registration failed. Try checking your input fields.");
        }
    };
            ...
            <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-3xl p-8 shadow-xl space-y-6">
                ...
                <div className="text-center space-y-2">
                    ...
                    <p className="text-sm text-slate-550 dark:text-slate-400">
                        Join InkWeave to start creating your articles
                    </p>
                </div>

                {/* Error Notifications */}
                {(localError || authError) && (
                    <div className="p-3.5 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/30 rounded-xl text-sm text-red-655 dark:text-red-400 font-medium">
                        {localError || authError}
                    </div>
                )}
